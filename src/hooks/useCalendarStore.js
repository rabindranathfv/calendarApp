...
export const useCalendarStore = () => {
  ...
  const startSavingEvent = async (calendarEvent) => {
    try {
      // TODO: Update Event
      if (calendarEvent.id) {
        const { data } = await calendarApi.put(`/events/${calendarEvent.id}`, {
          ...calendarEvent,
          user,
        });
      ...
      console.log(error);
      Swal.fire(
        "Error al guardar o actualizar un evento",
  ...
  const startDeleteEvent = async () => {
    try {
      ...
      const ModalDelete = await Swal.fire({
        ...
        icon: "info",
        html: `Desea eliminar el evento <b>${title}?</b>`,
